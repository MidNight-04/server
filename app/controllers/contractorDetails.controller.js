...
exports.saveContractorDetails = (req, res) => {
  let gstFiles = [];
  let panFiles = [];
  let aadharFiles = [];
  ...
  const dealerDetails = new ContractorDetails(query);
  ContractorDetails.findOne({ id: req.body.id }, (err, dealer) => {
    if (err) {
...
exports.getContractorDetails = (req, res) => {
  let id = req.body.id;
  let gstFiles = [];
  let panFiles = [];
  let bankDetailsFiles = [];
  ...
  if (id) {
    ...
    ContractorDetails.find({}, async (err, dealer) => {
      if (err) {
        res
          .status(500)
          .send({ message: "The requested data could not be fetched" });
...
exports.updateContractorDetails = (req, res) => {
  let gstFiles = [];
  let panFiles = [];
  let aadharFiles = [];
  let bankDetailsFiles = [];
...
exports.getAllContractors = (req, res) => {
    ContractorDetails.find({}, (err, dealers) => {
    if (err) {
      res.status(500).send({
    ...
    res
      .status(200)
      .send({ message: "List of contractors fetched successfuly", data: dealers });
    return;
  });
};
